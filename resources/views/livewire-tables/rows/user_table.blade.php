    ...
    <div class="text-nowrap d-flex align-items-center">
        ...
        <div class="d-inline-block align-top custom-space">
            <a href="{{route('users.show', $row->id)}}"
               class="text-primary-800">{{$row->full_name}}</a>
            <span class="d-block text-muted fw-bold h-0">{{$row->email}}</span>
        </div>
    </div>
...
<x-livewire-tables::bs5.table.cell class="text-center">
    <span class="badge badge-light-success">{{$row->plan_name}}</span>
</x-livewire-tables::bs5.table.cell>
...
<x-livewire-tables::bs5.table.cell>
    ...
    <label class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
        <input name="email_verified" data-id="{{$row->id}}" class="form-check-input user-is-verified"
               type="checkbox"
               value="1" {{$checked}}>
...
<x-livewire-tables::bs5.table.cell>
    <div class="d-flex justify-content-center">
        @if($row->is_active)
            <a data-turbo="false" href="javascript:void(0)" data-id="{{ $row->id }}" class="btn btn-sm btn-primary user-impersonate">
...
<x-livewire-tables::bs5.table.cell>
    ...
    @endphp
    <label class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
        <input type="checkbox" name="is_active" class="form-check-input user-active"
...
<x-livewire-tables::bs5.table.cell>
    <div class="edit d-inline-block me-1">
        <a title="{{ __('messages.common.edit') }}" href="{{ route('users.edit', $row->id) }}"
           class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 user-edit-btn" data-id="{{$row->id}}"
    ...
    <div class="delete d-inline-block">
        <a title="{{ __('messages.common.delete') }}" href="javascript:void(0)" data-id="{{ $row->id }}"
           class="btn user-delete-btn btn-icon btn-bg-light text-hover-danger btn-sm" data-turbolinks="false">
                        <span class="svg-icon svg-icon-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24"
                             version="1.1">
