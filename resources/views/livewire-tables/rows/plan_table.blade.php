...
<x-livewire-tables::bs5.table.cell>
    @if ($row->frequency == 1)
        <a class="badge badge-light-info fs-7 m-1">
...
<x-livewire-tables::bs5.table.cell>
    @if ($row->is_default == 1)
    <span class="badge badge-light-success">{{__('messages.plan.default_Plan')}}<span>
    @else
...
<x-livewire-tables::bs5.table.cell>
    <div class="edit d-inline-block me-1">
        <a title="{{ __('messages.common.edit') }}" href="{{ route('plans.edit', $row->id) }}"
    ...
    <div class="delete d-inline-block">
        <a title="{{ __('messages.common.delete') }}" href="javascript:void(0)" data-id="{{ $row->id }}"
           class="btn plan-delete-btn btn-icon btn-bg-light text-hover-danger btn-sm" data-turbolinks="false">
                        <span class="svg-icon svg-icon-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24"
                             version="1.1">
